...
class CmuDatasetPoseTarget(Dataset):
    def __init__(self, poses_3d, poses_2d, cams):
        assert poses_3d is not None
        self.poses_3d = poses_3d
        self.poses_2d = poses_2d
        self._cams = np.concatenate(cams)
        self.cams_num = len(self._cams)


    def __getitem__(self, index):
        out_pose_3d = self.poses_3d[index]
        out_pose_3d = torch.from_numpy(out_pose_3d).float()

        out_pose_2d = self.poses_2d[index]
        out_pose_2d = torch.from_numpy(out_pose_2d).float()

        out_cam = self._cams[index % (self.cams_num-1)]
        return out_pose_3d, out_pose_2d, '', out_cam
